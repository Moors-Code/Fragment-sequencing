...
V2_close <- subset(V2, idents = "close")
V2_far <- subset(V2, idents = "far")

########## Plot number of Features per sample and feature area and compare with sphere-seq data ##########
###Visium sample 1
V1_nFeature_RNA_close <- median(V1_close@meta.data$nFeature_Spatial)
V1_nFeature_RNA_far <- median(V1_far@meta.data$nFeature_Spatial)

###Visium sample 2 
V2_nFeature_RNA_close <- median(V2_close@meta.data$nFeature_Spatial)
V2_nFeature_RNA_far <- median(V2_far@meta.data$nFeature_Spatial)
...
close_SpS1 <- subset(SpS1, idents = "close") 
far_SpS1 <- subset(SpS1, idents = "far") 
median_nFeature_RNA_close_SpS1 <- median(close_SpS1@meta.data$nFeature_RNA)
median_nFeature_RNA_far_SpS1 <- median(far_SpS1@meta.data$nFeature_RNA)

Idents(SpS2) <- "Mets_distance"
close_SpS2 <- subset(SpS2, idents = "close") 
far_SpS2 <- subset(SpS2, idents = "far") 
median_nFeature_RNA_close_SpS2 <- median(close_SpS2@meta.data$nFeature_RNA)
median_nFeature_RNA_far_SpS2 <- median(far_SpS2@meta.data$nFeature_RNA)

###Plot nFeature per area and sample in barplot 
sample <- c("S1_SpS", "S1_Visium","S2_SpS","S2_Visium")
close_values <- c(median_nFeature_RNA_close_SpS1,V1_nFeature_RNA_close ,median_nFeature_RNA_close_SpS2,V2_nFeature_RNA_close)
far_values <- c(median_nFeature_RNA_far_SpS1, V1_nFeature_RNA_far,median_nFeature_RNA_far_SpS2,V2_nFeature_RNA_far)

df_close <- data.frame(sample,close_values)
colnames(df_close) <- c("Sample","nFeature")
df_close$nFeature <- -(df_close$nFeature)
df_far <- data.frame(sample,far_values)
colnames(df_far) <- c("Sample","nFeature")

df <- rbind(df_close,df_far)

p <- ggplot(df, aes(x=nFeature, y=Sample,fill = nFeature)) + theme_classic() +
  geom_bar(stat = "identity",position = "identity",aes(fill = ifelse(nFeature<0,"close","far"))) + 
  theme(axis.text = element_text(size = 15)) +
  theme(axis.title= element_text(size = 25)) + 
  ggtitle("nFeature of Visium and SpS expreiments") + xlab("nFeatures") + 
  ylab("Sample")  +

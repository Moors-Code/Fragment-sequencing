########## Part 2.6.2: Cell type abundance between close and far distances from metastatic sites##########
#This part compares cell type abundance of close and far distances from metastatic sites 
...
top <- top %>% 
  mutate(
    Expression = case_when(logFC >= 0.3 & PValue <= 0.05 ~ "High in noMets",
                           logFC <= -0.3 & PValue <= 0.05 ~ "High in Mets",
                           TRUE ~ "Non sig.")
  )
...
top <- top %>% 
  mutate(
    Expression = case_when(logFC >= 0.4 & PValue <= 0.05 ~ "High in noMets",
                           logFC <= -0.4 & PValue <= 0.05 ~ "High in Mets",
                           TRUE ~ "Non sig.")
  )
...
########## Plot proportions in boxplots close vs. far of cell types ##########
...
########## Plot proportions in boxplots close vs. far of monocytes subtypes ##########
###create table of cell type proportions per sample 

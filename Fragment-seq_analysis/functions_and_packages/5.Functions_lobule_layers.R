...
add_zonation_coefficient <- function(
  ...
  ###Generate pseudobulks of fragments with portal and central landmark genes 
  ...
  #Average expression of CV and PV genes in fragments, take raw counts and log normalize them beforehand
  ...
  endo_norm <- NormalizeData(endo, normalization.method = "LogNormalize",
                             scale.factor = 10000,
                             margin = 1, assay = "RNA")
  endo_av_PV <- AverageExpression(endo_norm, features = PV_genes,assays = "RNA", slot = "data", group.by = "fragment")
  endo_av_CV <- AverageExpression(endo_norm, features = CV_genes,assays = "RNA", slot = "data", group.by = "fragment")
  ...
  #normalize second time by dividing each gene by the maximum level from all fragments, this makes sure that each gene gets the same weight   
  ...
  #calculate colSums to get sum of counts for CV and PV genes from each fragment
  ...
  #Calculate 
  Zonation_score_endo_per_fragment <- endo_av_PV_df_total["Total",]/(endo_av_PV_df_total["Total",]+endo_av_CV_df_total["Total",])
  
  #Rescale ZCs to that 0 is most central, 1 is most portal (ZC = (ZC-min/ZC)) / (max(ZC)-min(ZC))
  minimal_zC <- min(Zonation_score_endo_per_fragment)
  maximal_zC <- max(Zonation_score_endo_per_fragment)
  Zonation_score_endo_per_fragment <- sort(Zonation_score_endo_per_fragment)
  Zonation_score_endo_per_fragment.df <- as.data.frame(Zonation_score_endo_per_fragment)
  colnames(Zonation_score_endo_per_fragment.df) <- "zonation_coordinate"
  Zonation_score_endo_per_fragment.df2 <- matrix(0,nrow(Zonation_score_endo_per_fragment.df), ncol(Zonation_score_endo_per_fragment.df), 
                                               dimnames = list(rownames(Zonation_score_endo_per_fragment.df), colnames(Zonation_score_endo_per_fragment.df)))  
  for (i in 1:nrow(Zonation_score_endo_per_fragment.df)){          
    original.val <- Zonation_score_endo_per_fragment.df[i,]         
    adapted_value <- (original.val-minimal_zC)/(maximal_zC-minimal_zC)           
    Zonation_score_endo_per_fragment.df2[i] <- adapted_value                            
  }
  Zonation_score_endo_per_fragment.df2 <- as.data.frame(Zonation_score_endo_per_fragment.df2)
  
  ###Add ZC to Seurat object 
  #remove the 'RNA.' from the Barcode name 
  rownames(Zonation_score_endo_per_fragment.df2) <- gsub(pattern = "RNA.", x = rownames(Zonation_score_endo_per_fragment.df2), replacement = "")
  
  current.cluster.ids <- rownames(Zonation_score_endo_per_fragment.df2)
  new.cluster.ids <- as.numeric(Zonation_score_endo_per_fragment.df2$zonation_coordinate)
  seurat_object$zonation_coordinate <- plyr::mapvalues(x = seurat_object$fragment, from = current.cluster.ids, to = new.cluster.ids)
  #save Seurat object 
  saveRDS(seurat_object,file = output_seurat_file_name)
  return(seurat_object)
}  

###Function to remove fragments without LECs because there is no ZC
remove_fragments_noLECs <- function(
  seurat_object_path,
  seurat_object_new_path
){
  seurat_object <- readRDS(seurat_object_path)
  #check for fragments that don't have zonation coordinate 
  vector_all_bc <- as.character(as.data.frame(table(seurat_object$fragment))$Var1)
...
boxplot_zonation_genes_LECs_injected <- function(
  ...
  cellType_norm <- NormalizeData(cellType, normalization.method = "LogNormalize",
                                 scale.factor = 10000,
                                 margin = 1, assay = "RNA")
  #per zonation coordinate = per fragment 
  ...
  p <- ggplot(m, aes(x=lobule_layer, y=value, fill=variable)) +  theme_classic() +
    ...
    ggtitle(paste0(gene_oi," expression in LECs - fragment-seq")) + xlab("Lobule layer (CV-PV)") + 
    ylab("Average expression per fragment") +
...
boxplot_zonation_genes_KCs_injected <- function(
  ...
  p <- ggplot(m, aes(x=lobule_layer, y=value, fill=variable)) +  theme_classic() +
    geom_boxplot(fill = color_oi,outlier.shape = NA) + theme(axis.text = element_text(size = 30))  + theme(axis.text.x = element_text(angle = 90)) +
    geom_jitter(color="black", size=1, alpha=0.9) +theme(axis.title= element_text(size = 25)) + 
    ggtitle(paste0(gene_oi," expression in KCs - fragment-seq")) + xlab("Lobule layer (CV-PV)") + 
    ylab("Average expression per fragment") +
    theme(plot.title = element_text(size = 25, face = "bold")) + theme(legend.text = element_text(size = 30),
                                                                       legend.title= element_text(size = 30)) + 
    guides(fill=guide_legend(title="Gene"))  + 
...
boxplot_zonation_genes_LECs_WT <- function(
  ...
  p <- ggplot(m, aes(x=vein, y=value, fill=variable)) + theme_classic() +
    ...
    ggtitle(paste0(gene_oi," expression in LECs - Fragment-seq WT")) + xlab("Lobule layer") + 
    ylab("Average expression per fragment") + theme(axis.title= element_text(size = 25)) +
    theme(plot.title = element_text(size = 25, face = "bold")) + theme(legend.text = element_text(size = 30),
                                                                       legend.title= element_text(size = 30)) + 
...
boxplot_zonation_genes_KCs_WT <- function(
  ...
  p <- ggplot(m, aes(x=vein, y=value, fill=variable)) + theme_classic() +
    geom_boxplot(fill = color_oi,outlier.shape = NA) + theme(axis.text = element_text(size = 30))  + theme(axis.text.x = element_text(angle = 90)) +
    geom_jitter(color="black", size=1, alpha=0.9) +
    ggtitle(paste0(gene_oi," expression in Kupffer - Fragment-seq WT")) + xlab("Lobule layer") + 
    ylab("Average expression per fragment") + theme(axis.title= element_text(size = 25)) +
    theme(plot.title = element_text(size = 25, face = "bold")) + theme(legend.text = element_text(size = 30),
                                                                       legend.title= element_text(size = 30)) + 
    guides(fill=guide_legend(title="Gene")) + ggsignif::geom_signif(comparisons = list(c("CV", "PV")),

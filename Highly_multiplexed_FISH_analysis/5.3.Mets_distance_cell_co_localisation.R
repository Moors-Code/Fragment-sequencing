########## Part 5.3 Mets distance cell type co-localisation analysis ##########
#This part does cell co-localisation analysis comparing close and far areas 
...
source("./functions_and_packages/4.Functions_cell_co_localisation.R")

###Load data 
merged <- readRDS(file = "./data_files_generated/Resolve_seurat_anno.rds")

#only consider samples with visible metastasis 
mets_samples <- subset(merged, idents = c("mets"))
srt <- mets_samples
...
#remove the metastatic cells, not informative in this case 
pD <- pD[!pD$annotation=="Metastasis",]
...
###Now this is reduced to only the close area and contains a column with the difference with regards to the far area
...
ggsave("./functions/5.3/Contacts_Mets.pdf",width = 12, height = 10)
ggsave("./functions/5.3/Contacts_Mets.svg",width = 12, height = 10)

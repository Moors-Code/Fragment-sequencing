...
top <- top %>% 
  mutate(
    Expression = case_when(logFC >=0.5 & PValue <= 0.05 ~ "High in noMets",
                           logFC <= -0.5 & PValue <= 0.05 ~ "High in Mets",
                           TRUE ~ "Non sig.")
  )
...
p + ggsave("./figures/5.1/Mono_LM_vulcano_close_far_Resolve.pdf",width = 12, height = 10)
p + ggsave("./figures/5.1/Mono_LM_vulcano_close_far_Resolve.svg",width = 12, height = 10)  
